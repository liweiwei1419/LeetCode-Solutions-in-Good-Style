...
public class Solution3 {
    ...
    private void dfs(char[][] board, int i, int j, TrieNode parentNode, boolean[][] visited, List<String> res) {
        Character letter = board[i][j];
        TrieNode currNode = parentNode.next.get(letter);

        if (currNode.isWord != null) {
            res.add(currNode.isWord);
            // 找到了就从字典中删除
            currNode.isWord = null;
            // 思考这里为什么不可以 return
        ...
        for (int[] direction : DIRECTIONS) {
            ...
            if (inArea(newX, newY) && currNode.next.containsKey(board[newX][newY])) {
                dfs(board, newX, newY, currNode, visited, res);
            }
        }
